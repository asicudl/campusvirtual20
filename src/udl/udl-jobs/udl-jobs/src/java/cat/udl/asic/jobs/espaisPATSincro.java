...
public class espaisPATSincro implements Job {
	...
	private static String plantillaId= "plantillaPAT";
	...
	public void init() {
	    ...
	    log.debug("Executem init() de espaisPATSincro");
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		...
		log.info("Executant la tasca de creació de espais PAT usant CM");
        ...
        try {
        		...
        		while (rst.next()) {
	            		...
	            		if (retorn.equals("error"))
	            		{
	            			...
	            			log.info("Espai pat "+idSite+" creat correctament");
	        				...
	        				while (iter.hasNext()) {
	        							...
	        							// a l'espai només afegim els grups pat
	        							if (categoria.startsWith("GRUP_PAT") ) {
	            			...
	            			if (retorn2.equals("error"))
	                		{
	                			...
	                			log.info("Esborrem l'espai pat  "+idSite);
	            				try{	            					
	            					...
	            					log.error("EXCEPCIO (esborrant espai pat "+idSite+")");
        	...
        	log.error("EXCEPCIO SQL(espaisPATSincro) ");
        	log.error("EXCEPCIO SQL "+e);
        }
        catch (Exception ex) {
            log.error("EXCEPCIO (espaisPATSincro) ");
	...
	private String createSite(String idEspai, String nomEspai, String term) throws JobExecutionException {
		...
		try {
				...
				resourcePropertiesEdit.addProperty("tipus_espai", "pat");
				...
				log.error("EXCEPCIO (createEspaiPAT)");
				log.error(e.getClass().getName() + " :: " + e.getMessage());
				return "error";
			} finally {
				log.debug("Deshabilitem el security advisor");
